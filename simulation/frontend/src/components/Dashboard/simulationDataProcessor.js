...
export const processSimulationData = (results) => {
    ...
    const nodeStats = Object.entries(results.summary_stats).map(([nodeId, stats]) => {
      ...
      // Calculate utilization properly using service time and flow time
      const utilization = stats.service_time_stats?.mean && stats.flow_time_stats?.mean
        ? (stats.service_time_stats.mean / stats.flow_time_stats.mean)
        : 0;
      ...
      return {
        ...
        throughput: {
          ...
          completed: results.transactions.filter(t => 
            t.node_id === numericNodeId && t.exit_time !== null
          ).length
        }
      };
    });
...
const calculatePercentiles = (values) => {
  ...
  // Helper function to calculate utilization from stats
  const calculateUtilization = (nodeStats) => {
    const avgServiceTime = nodeStats.service_time_stats.mean;
    const totalCustomers = nodeStats.queue_size_stats.max;  // Using max as proxy for total
    if (!avgServiceTime || !totalCustomers) return 0;
    
    // Basic utilization calculation
    return Math.min(1, (avgServiceTime * totalCustomers) / 1440); // 1440 = 24 hours in minutes
  };
  
  // Calculate system-wide statistics
  const calculateSystemStats = (transactions, timeField) => {
    const times = transactions
      .map(t => t[timeField])
      .filter(t => t !== null && t !== undefined);
      
    if (times.length === 0) return { mean: 0, percentiles: { '90': 0 } };
  ...
  // Filter data based on node and time window
