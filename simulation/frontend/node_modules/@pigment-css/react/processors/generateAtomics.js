...
var GenerateAtomicsProcessor = class extends chunk7S25MALX_js.BaseProcessor {
  ...
  build(values) {
    ...
    if (param.kind !== shared.ValueType.CONST) {
      ...
      classes.forEach(({ className, css: cssObject }) => {
        ...
        const emotionClass = chunk4FIMMGNT_js.css(cssObject);
        const cssText = chunk4FIMMGNT_js.cache.registered[emotionClass];
        const rules = {
          [`.${className}`]: {
            cssText,
  ...
  doRuntimeReplacement() {
    ...
    this.replacer(t.callExpression(importName, [chunkY7JG2JA7_js.valueToLiteral(this.runtimeConfig)]), true);
  }
};
