...
var CssProcessor = class extends chunk7S25MALX_js.BaseProcessor {
  ...
  generateArtifacts(styleObjOrTaggged, ...args) {
    ...
    const cssClassName = chunk4FIMMGNT_js.css(styleObjOrTaggged, ...args);
  ...
  handleCall([, ...callArgs], values) {
    ...
    callArgs.forEach((callArg) => {
      let styleObj;
      if (callArg.kind === shared.ValueType.LAZY) {
        styleObj = values.get(callArg.ex.name);
      } else if (callArg.kind === shared.ValueType.FUNCTION) {
        const { themeArgs } = this.options;
        const value = values.get(callArg.ex.name);
        styleObj = value(themeArgs);
      }
      if (styleObj) {
        deepMerge__default.default(mergedStyleObj, styleObj);
      }
    });
